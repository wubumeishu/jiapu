...
function getPanBounds() {
  ...
  // 计算允许平移的范围
  // minPanX: 内容最右边对齐视口最左边（向左拖动）
  // maxPanX: 内容最左边对齐视口最右边（向右拖动）
  const minPanX = contentRight - viewportW  // 负值，向左拖
  const maxPanX = -contentLeft              // 正值，向右拖

  const minPanY = contentBottom - viewportH  // 负值，向上拖
  const maxPanY = -contentTop                // 正值，向下拖
