<template>
  <div class="genealogy-tree-container">
    ...
    <div
      class="tree-viewport"
      ref="viewportRef"
      :style="{ overflow: (needVerticalScroll || needHorizontalScroll) ? 'auto' : 'hidden' }"
    >
      <div 
        ...
        :style="{ 
          ...
          marginTop: scaleOffsetY + 'px',
          marginLeft: scaleOffsetX + 'px'
        }"
      >
      <svg
        class="tree-svg"
...
// 计算缩放后的偏移量以实现居中
const scaleOffsetX = computed(() => {
  if (scale.value >= 1) return 0
  const viewportW = VIEWPORT_WIDTH
  const scaledW = scaledWidth.value
  return (viewportW - scaledW) / 2
})

const scaleOffsetY = computed(() => {
  if (scale.value >= 1) return 0
  const viewportH = 700
  const scaledH = scaledHeight.value
