...
function getPanBounds() {
  ...
  let maxY = -Infinity
  layoutMap.forEach((node: any) => {
    if (node.visible || !collapsedNodes.value.has(node.id)) {
  ...
  const contentW_actual = contentRight - contentLeft
  const contentH_actual = contentBottom - contentTop

  // 计算溢出量（考虑内容实际位置）
  const overflowX = Math.max(0, contentW_actual - viewportW)
  const overflowY = Math.max(0, contentH_actual - viewportH)

  // 允许平移的范围：
  // - 最小panX：内容最左边对齐视口左边（可以向左拖动看到左边内容）
  // - 最大panX：内容最右边对齐视口右边（可以向右拖动看到右边内容）
  const minPanX = contentLeft - viewportW  // 负值，允许向左拖
  const maxPanX = contentLeft              // 正值或0，允许向右拖

  const minPanY = contentTop - viewportH   // 负值，允许向上拖
  const maxPanY = contentTop               // 正值或0，允许向下拖
