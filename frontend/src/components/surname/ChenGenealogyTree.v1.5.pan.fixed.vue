...
function getPanBounds() {
  ...
  // 内容超出视口的量
  const overflowX = Math.max(0, contentW - viewportW)
  const overflowY = Math.max(0, contentH - viewportH)

  // 允许平移的范围：从完全显示左侧到完全显示右侧
  // 当 contentW <= viewportW 时，不允许平移（内容完全在视口内）
  // 当 contentW > viewportW 时，允许左右各平移 overflowX
  return {
    minPanX: -overflowX,
    maxPanX: 0,
    minPanY: -overflowY,
