...
function getPanBounds() {
  ...
  const viewportH = 700
  const EXTRA_PAN = 100 // 额外允许平移的距离（像素）
  ...
  // 计算平移范围
  // 默认居中时，内容应该在视口中央
  // 允许向四个方向额外移动 EXTRA_PAN 像素

  // 水平方向：
  // - 内容最右边可以在视口最左边左侧 EXTRA_PAN 像素（向左拖）
  // - 内容最左边可以在视口最右边右侧 EXTRA_PAN 像素（向右拖）
  const minPanX = contentRight - viewportW - EXTRA_PAN
  const maxPanX = -contentLeft + EXTRA_PAN

  // 垂直方向：
  // - 内容最下边可以在视口最上边上方 EXTRA_PAN 像素（向上拖）
  // - 内容最上边可以在视口最下边下方 EXTRA_PAN 像素（向下拖）
  const minPanY = contentBottom - viewportH - EXTRA_PAN
  const maxPanY = -contentTop + EXTRA_PAN

  // 如果内容在视口内且不需要额外移动，返回中心位置
  if (contentW <= viewportW && contentH <= viewportH) {
    // 居中显示时，panX 和 panY 应该让内容居中
    const centerX = -(contentW - viewportW) / 2
    const centerY = -(contentH - viewportH) / 2
    return {
      minPanX: centerX - EXTRA_PAN,
      maxPanX: centerX + EXTRA_PAN,
      minPanY: centerY - EXTRA_PAN,
      maxPanY: centerY + EXTRA_PAN
    }
  }
...
function clampPanX(value: number) {
  ...
  return Math.min(maxPanX, Math.max(minPanX, value))
}
